...
app.use(cors({
  origin: ['https://ecub-bot4.vercel.app', 'http://localhost:5173'],
  credentials: true
}));
...
// Decode and write Google credentials to a temporary file
const googleCredentials = Buffer.from(process.env.GOOGLE_APPLICATION_CREDENTIALS_BASE64, 'base64').toString('utf8');
const tempCredentialsPath = path.join(os.tmpdir(), 'google-credentials.json');
fs.writeFileSync(tempCredentialsPath, googleCredentials);
...
app.post('/api/cart/add', async (req, res) => {
  try {
    ...
    // For example, you might want to store this in a database or in-memory store
